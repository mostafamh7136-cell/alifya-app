...
// Curated sources that were individually verified as human recordings of Standard Arabic.
...
const DIALECT_CODES = new Set([
  "ajp", "ary", "arz", "apc", "afb", "acm", "acq", "aeb", "aao", "abh", "ars", "avl", "shu",
]);
...
function isClearlyMsaWikimediaUrl(raw: string) {
  ...
  return /(?:^|[/_-])Ar-(?:[^/]+)$/i.test(decoded) || /_\(ar\)-/i.test(decoded);
}
...
async function findVerifiedMsaAudio(text: string): Promise<string | null> {
  ...
  try {
    ...
    const rawSources = [...doc.querySelectorAll("audio source,a[href]")]
    ...
    for (const raw of rawSources) {
      const url = raw.startsWith("//")
        ...
        : raw.startsWith("http")
          ? raw
          : new URL(raw, "https://en.wiktionary.org").href;
    ...
    // A failed remote lookup is treated as unavailable, never as permission to synthesize speech.
  }
...
export default function AudioButton({ text, lang = "ar", label, compact = false }: Props) {
  ...
  const speak = async () => {
    ...
    // Clear the previous URL before resolving the next phrase so a missing lookup
    // can never accidentally replay the previous lesson's audio.
    audio.pause();
  ...
  const title = source === "human-msa"
    ? "Verified human MSA recording"
    : source === "unavailable"
      ? "No verified human MSA recording available yet"
      : "Human MSA recording";
  ...
  return (
    <>
      <audio
        ...
        data-audio-human={source === "human-msa" ? "true" : "false"}
        aria-hidden="true"
      />
      <button
        type="button"
