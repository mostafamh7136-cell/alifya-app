...
export default function AudioButton({text,lang="ar",label,compact=false}:Props){
 ...
 return <><audio ref={audioRef} preload="none" onEnded={()=>setBusy(false)} onError={()=>{setBusy(false);setError(true)}} data-audio-language={lang} data-audio-source={human?"human-wikimedia":"browser-fallback"} aria-hidden="true"/><button type="button" onClick={speak} className={`audio-btn ${compact?"audio-btn-compact":""}`} aria-label={`${displayLabel}: ${text}`} aria-pressed={busy} title={human?"Human recording":"Browser voice fallback"}><span aria-hidden>{busy?"■":"🔊"}</span>{!compact&&<span>{busy?(human?"Playing human recording":"Playing"):(error?"Try again":displayLabel)}</span>}</button></>;
}
